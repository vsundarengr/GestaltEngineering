...
def getPsychroPropertiesFromTdRH(Td, RH, ptotal = 101.3025):
   ...
   # print('Saturation properties ', props)
    ...
    if ( np.absolute(pv) > 1e-6):
        Tdew = getSaturationTemperature(pv) - Ta
    else:
        Tdew = -100
    specvol = (1+omega)*(Td+Ta)*1/( (ptotal-pv)/0.287 + pv/0.462)
    psychroprops = {'Tdry': Td, "RH": RH, 'omega': omega, 'pv': pv, 'Twet': Twet, 'Tdew': Tdew, 'h': h, 'specvol': specvol, 'saturationProperties': props}
    return psychroprops

def getPsychroPropertiesFromTdTdew(Td, Tdew, ptotal = 101.3025):
    Cpa = 1.005
    Ta = 273.15
    propsTd = getSaturationProperties(Td)
    propsTdew = getSaturationProperties(Tdew)
    RH = propsTdew['Psat']/propsTd['Psat']
    RHp = RH*100
    pv = propsTdew['Psat']
    omega = 0.6220746*pv/(ptotal - pv)
    hg = propsTd['hg']
    ...
    Twet = Td*np.arctan(0.151977*(RHp+8.313659)**(0.5)) + np.arctan(Td+RHp)-np.arctan(RHp - 1.676331) + 0.00391838*((RHp)**(1.5))*np.arctan(0.023101*RHp)-4.686035
    specvol = (1+omega)*(Td+Ta)*1/( (ptotal-pv)/0.287 + pv/0.462)
    psychroprops = {'Tdry': Td, "RH": RH, 'omega': omega, 'pv': pv, 'Twet': Twet, 'Tdew': Tdew, 'h': h, 'specvol': specvol, 'saturationProperties': propsTd}
    return psychroprops


def getPsychroPropertiesFromTdOmega(Td, omega, ptotal = 101.3025):
    Cpa = 1.005
    Ta = 273.15
    propsTd = getSaturationProperties(Td)
...
def getPsychroPropertiesFromTdTwet(Td, Twet, ptotal = 101.3025):
    Cpa = 1.005
    Ta = 273.15
    propsTd = getSaturationProperties(Td)
    propsTwet = getPsychroPropertiesFromTdRH(Twet, 1, ptotal)
    hg1 = propsTd['hg']
    ...
    specvol = (1+o1)*(Td+Ta)*1/( (ptotal-pv)/0.287 + pv/0.462)
    RH = pv/propsTd['Psat']
    psychroprops = {'Tdry': Td, "RH": RH, 'omega': o1, 'pv': pv, 'Twet': Twet, 'Tdew': Tdew, 'h': h, 'specvol': specvol,'saturationProperties': propsTd}
    return psychroprops

...
defld = 10*math.round(100*math.random(delta1+0.2*dh, delta1+0.9*dh)); // not enough velocity to go up the ramp
...
v = math.sqrt( k/m*defl*defl - 2*g*H);
t = math.sqrt(2*H/g);
x = v*t;

vy = math.sqrt(2*g*H);

vs = math.sqrt(v*v + vy*vy);
angle = math.atan(vy/v)*180/math.pi;

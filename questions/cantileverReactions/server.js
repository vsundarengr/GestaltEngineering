...
const generate = () => {
   ...
   console.log(`After 1: Force is ${force} while moment is ${moment}`);
    ...
    if (numForces === 1) {
        ...
        if ( math.abs(H1) > 0 ) {
            htmlString += `The magnitude of the force at B is ${Fmag} ${unitsForce} and acts at angle ${angleString} with the positive x-axis. The horizontal distance between O and A is ${L1} ${unitsDist} and the height of the post AB is ${math.abs(H1)} ${unitsDist}. `;
        } 
        else {
            htmlString += `The magnitude of the force at A is ${Fmag} ${unitsForce} and acts at angle ${angleString} with the positive x-axis. The horizontal distance between O and A is ${L1} ${unitsDist}. `;
        }
    }
    if ( numForces > 1) {
        htmlString += `The magnitudes of the forces at ${fpt} are respectively ${forceString} and acts at an angle ${angleString} with the positive x-axis. The horizontal distances of ${fpt} from O are respectively  ${distString} ${unitsDist}. `;
        if ( math.abs(H1) > 0) {
    ...
    const data= {
        ...
        correct_answers: {
            Ox : reactionForce[0],
            Oy : reactionForce[1],
            reactionMoment: reactionMoment[2]
        },
